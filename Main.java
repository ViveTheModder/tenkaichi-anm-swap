			...
			public void actionPerformed(ActionEvent e) 
			{
				...
				else
				{
					try 
					{
						...
						/* RAF has no flush() method, so the only way to "reopen a RAF" is to make a new instance of the same goddamn file
						otherwise, Java will bitch at you because the stream is closed, when in reality the stream has to be closed for files to be overwritten */
						AnmPak[] newPaks = 
						{
							new AnmPak(new File(paks[0].getAbsPath())),
							new AnmPak(new File(paks[1].getAbsPath()))
						};
						cmd.Main.swap(newPaks[0],newPaks[1],anmIds);
